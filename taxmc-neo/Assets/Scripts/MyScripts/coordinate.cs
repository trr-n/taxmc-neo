...
namespace trrne.Bag
{
    public static class Coordinate
    {
        ...
        => t.position = new(
            minX is null ? t.position.x : Mathf.Clamp(t.position.x, (float)minX, (float)maxX),
        ...
        => t.rotation = Quaternion.Euler(
            x is null ? t.localScale.x : (float)x,
            y is null ? t.localScale.y : (float)y,
            z is null ? t.localScale.z : (float)z
        );
