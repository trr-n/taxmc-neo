...
namespace trrne.Box
{
    public static class Paths
    {
        ...
        => $"{UniApp.dataPath}/{name}";

        public static string PersistantPath(string name)
        => $"{UniApp.persistentDataPath}/{name}";
    }
    ...
    public class App
    {
        ...
        public static float FPS(int digit)
        => MathF.Round(1 / Time.unscaledDeltaTime, digit);
        ...
        => Numcs.Cutail(FPS(digit: 0));

        public static void SetCursorStatus(
            CursorAppearance appear,
            CursorRangeOfMotion rangeOfMotion
        )
        {
        ...
        => (truth = UniApp.platform) == platform;

        public static Vector2 GetScreenInfo(KindaScreen kinda)
        => kinda switch
        {
            KindaScreen.Resolution => new(
